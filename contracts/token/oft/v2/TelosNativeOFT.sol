...
// NativeOFTV2
contract TelosNativeOFT is OFTV2, ReentrancyGuard {
    ...
    function withdraw(uint _amount) external nonReentrant {
        require(balanceOf(msg.sender) >= _amount, "TelosOFT: Insufficient balance.");
        _burn(msg.sender, _amount);
        (bool success, ) = msg.sender.call{value: _amount}("");
        require(success, "TelosOFT: failed to unwrap");
    ...
    function _send(
        ...
        require(amount > 0, "TelosOFT: amount too small");
    ...
    function _debitMsgSender(uint _amount) internal returns (uint messageFee) {
        ...
        if (msgSenderBalance < _amount) {
            require(msgSenderBalance + msg.value >= _amount, "TelosOFT: Insufficient msg.value");
    ...
    function _debitMsgFrom(address _from, uint _amount) internal returns (uint messageFee) {
        ...
        if (msgFromBalance < _amount) {
            require(msgFromBalance + msg.value >= _amount, "TelosOFT: Insufficient msg.value");
    ...
    function _creditTo(
        ...
        require(success, "TelosOFT: failed to _creditTo");
        return _amount;
    }
